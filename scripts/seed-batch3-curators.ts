// scripts/seed-batch3-curators.ts
// Seeds curators from Heard pages 4-5, LetsSubmit, and additional online sources
...
function extractSpotifyId(url: string): string | null {
  ...
  // Also handle user URLs — we'll skip those for playlists
  const userMatch = url.match(/user\/([a-zA-Z0-9]+)/);
  if (userMatch) return null; // user URLs don't have playlist IDs
  return null;
}

async function main() {
  const dataPath = join(__dirname, "..", "curators_seed_batch3.json");
  const raw = readFileSync(dataPath, "utf-8");
  const curators: Batch3Curator[] = JSON.parse(raw);

  console.log(`Seeding ${curators.length} batch3 curators...\n`);
