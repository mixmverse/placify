...
function RegisterForm() {
  ...
  const role = params.get("role") as "artist" | "curator" | null;
    ...
    <main className="flex min-h-full flex-1 items-center justify-center px-6 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
          ...
          {/* Role selection (if no role specified) */}
          {!role && (
          ...
          {/* Sign-up buttons (once role is selected) */}
          {role && (
            <div className="mt-6 space-y-3">
              <button
              ...
              <button
                onClick={() => signIn("credentials", { email: role === "curator" ? "curator@example.com" : "artist@example.com", callbackUrl: role === "curator" ? "/dashboard/curator" : "/dashboard/artist" })}
                className="w-full rounded-full bg-black px-5 py-3 text-sm text-white transition-colors hover:bg-zinc-800 dark:bg-white dark:text-black"
              >
                Continue as demo {role === "curator" ? "Curator" : "Artist"}
              </button>
            </div>
          )}
